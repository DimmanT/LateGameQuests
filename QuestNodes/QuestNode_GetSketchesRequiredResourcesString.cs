...
{
    public class QuestNode_GetSketchesRequiredResourcesString : QuestNode
    {
        [NoTranslate]
        ...
        public SlateRef<List<object>> markers;
        ...
        protected override void RunInt()
        {
			...
			foreach (MonumentMarker m in markers.GetValue(slate))
			{
				CalcResources(m,ref dictionary,ref list, ref list2 );
			}

			//rebuild list2 -> dict2
			Dictionary<ThingDef, int> dict2 = new Dictionary<ThingDef, int>();
			{
				foreach (List<Pair<ThingDef, int>> sublist in list2)
					foreach (Pair<ThingDef, int> pair in sublist)
					{
						dict2[pair.First] = pair.Second;
					}
			}
			...
			if (dict2.Any())
			{
				...
				{
					stringBuilder.AppendLine();
                    stringBuilder.Append("    - " + k.label + " x" + dict2[k]);
				}
		...
		protected void CalcResources(MonumentMarker value, ref Dictionary<ThingDef, int> dictionary, ref List<Pair<List<StuffCategoryDef>, int>> list, ref List<List<Pair<ThingDef, int>>> list2)
		{
